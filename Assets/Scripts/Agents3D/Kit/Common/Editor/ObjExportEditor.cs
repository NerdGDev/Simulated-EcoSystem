...
public class ObjExportEditor : ScriptableObject
{
		...
		using (StreamWriter sw = new StreamWriter(Path.Combine(folder, filename + ".mtl")))
		{
			foreach (KeyValuePair<string, ObjMaterial> kvp in materialList)
			{
				...
				if (kvp.Value.textureName != null)
				{
					...
					int stripIndex = kvp.Value.textureName.LastIndexOf(Path.PathSeparator);
					if (stripIndex >= 0)
						destinationFile = kvp.Value.textureName.Substring(stripIndex + 1).Trim();
					else
						destinationFile = Path.GetFileName(kvp.Value.textureName); ;
