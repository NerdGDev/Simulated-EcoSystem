//
// From https://forum.unity3d.com/threads/editor-want-to-check-all-prefabs-in-a-project-for-an-attached-monobehaviour.253149/
//
using UnityEngine;
using UnityEditor;
...
{
	[MenuItem("Kit/Project Search/Search For Components")]
	static void Init()
	{
	...
	enum eDetectionMode
	{
		...
		ComponentAndSubclasses,
	}
	eDetectionMode detectionMode = eDetectionMode.Default;
	...
	T EnumSelectionGrid<T>(T currentValue)
	{
		if (!typeof(T).IsEnum)
			throw new System.ArgumentException("EnumSelectionGrid must be used with an enum: " + typeof(T));

		string[] names = System.Enum.GetNames(typeof(T));

		return (T)(object)GUILayout.SelectionGrid((int)(object)currentValue, names, names.Length);
	}
	...
	void OnGUI()
	{
		...
		windowRect.x += 4;
		windowRect.width -= 7;
		editorMode = GUI.SelectionGrid(windowRect, editorMode, new[] { "Search for component usage", "Search for missing components" }, 2, "Window");
		...
		{
			case 0:
				selectedCheckType = GUILayout.SelectionGrid(selectedCheckType, new[] { "Check single component", "Check all components" }, 2);
				...
				detectionMode = EnumSelectionGrid(detectionMode);
				GUI.enabled = true;
				...
				if (GUILayout.Button("Check component usage"))
				{
					...
					switch (selectedCheckType)
					{
						case 0:
							...
							{
								if (detectionMode == eDetectionMode.ComponentAndSubclasses)
								{
									var obj = AssetDatabase.LoadMainAssetAtPath(prefab) as GameObject;
									if (obj != null && obj.GetComponentInChildren(targetComponent.GetClass(), true) != null)
										listResult.Add(prefab);
								}
								else
								{
									bool recursive = detectionMode == eDetectionMode.RecursiveDependencies;
									string[] single = new string[] { prefab };
									string[] dependencies = AssetDatabase.GetDependencies(single, recursive);
				...
				else
				{
					...
					if (GUILayout.Button("Select all"))
					{
						...
						foreach (string s in listResult)
							allObjects.Add(AssetDatabase.LoadMainAssetAtPath(s));
